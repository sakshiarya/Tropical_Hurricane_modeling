#################################################################
#################################################################
#Title: Fully Bayesian Hierarchical Models with all Covariates
...
filemcmc<-'/Users/sakshi/Documents/School/Research/Lindsey_Dissertation2021/Chapter 2 - Bayesian Damage/MCMC Chains/Full Bayes/March 31/Full Bayesian/All_covs'
...
#################################################################
#################################################################
#BUGS model using May/June Avg. for all covariates except sunspots
#Based on freq. model selection
#Used all covariates for TS-2 and for 3-5
...
data.nbpoi.selected <-list(
  ...
  hyper_Beta1 = matrix(c(rep(0,ncol(model.mat.x)),rep(1e-5,ncol(model.mat.x))),nrow=2,byrow=T), 
  hyper_Beta2 = matrix(c(rep(0,ncol(model.mat.x)),rep(1e-5,ncol(model.mat.x))),nrow=2,byrow=T), 
...
set.seed(22415)
clones.MCMC.final.FB.all.April2 <-dc.fit(data= data.nbpoi.selected, params= params.nb, model= model.bugs.onecov.nb, n.iter = n.iter, n.clones= n.clones, multiply="num_obs", n.chains= n.chains,n.adapt = n.adapt, n.update = 0)
save(clones.MCMC.final.FB.all.April2 ,file=file.path(filemcmc,'clones.MCMC.final.FB.all.April2.rda'))
...
set.seed(22415)
jags.MCMC.final.FB.all.April2 <-jags(data= data.nbpoi.selected, parameters.to.save= params.nb, model.file= model.bugs.onecov.nb, n.chains= n.chains, n.thin= n.thin, n.iter= n.iter, n.burnin= n.burnin)
save(jags.MCMC.final.FB.all.April2,file=file.path(filemcmc,'jags.MCMC.final.FB.all.April2.rda'))
